...
recorder Node -file [pwd]\\outputs\\{filename}.txt  -time -node 390 -dof 3 disp

integrator EQPath {arc_length} 9 node -dof 3 -tags 390
...
while {$i<$steps && $ret==0} {
	incr i
	set ret [analyze 1];
	puts  $fileID   "$i	[getTime]	[nodeDisp 390 3]	[getCurrentIterations]	$ret"
        puts "[expr [getTime]] [expr [nodeDisp 390 3]]"
